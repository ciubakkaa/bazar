<nav class="flex justify-between items-center px-5 py-4 md:px-8 max-w-[1200px] mx-auto">
  <a href="/" class="font-heading font-bold text-[28px] tracking-tight text-bazar-dark no-underline">
    ba<span class="inline-block bg-bazar-yellow px-2 py-0.5 rounded-md -rotate-2 ml-0.5">zar</span>
  </a>
  <div class="flex gap-3 items-center">
    <a href="/checklist" class="hidden md:inline-block text-bazar-gray-700 text-[15px] font-medium px-4 py-2 rounded-bazar-sm hover:bg-bazar-gray-100 transition-colors no-underline">
      Demo
    </a>
    <a href="#features" class="hidden md:inline-block text-bazar-gray-700 text-[15px] font-medium px-4 py-2 rounded-bazar-sm hover:bg-bazar-gray-100 transition-colors no-underline">
      Cum functioneaza
    </a>
    <a href="/register" class="bg-bazar-dark text-white font-semibold text-[15px] px-6 py-2.5 rounded-bazar-sm hover:-translate-y-0.5 transition-transform no-underline">
      Inregistreaza-te
    </a>
  </div>
</nav>

<!-- Hero -->
<section class="max-w-[1200px] mx-auto px-5 md:px-8 pt-8 md:pt-16 pb-10 grid grid-cols-1 md:grid-cols-2 gap-10 md:gap-16 items-center">
  <div>
    <h1 class="font-heading font-bold text-4xl md:text-[56px] leading-[1.1] tracking-tight mb-5">
      Tot ce trebuie sa stii
      <br />
      <span class="relative inline-block">
        inainte de facultate.
        <span class="absolute bottom-1 left-[-4px] right-[-4px] h-3.5 bg-bazar-yellow opacity-60 -z-10 rounded -rotate-1"></span>
      </span>
    </h1>
    <p class="text-lg text-bazar-gray-700 leading-relaxed mb-8 max-w-[460px]">
      Checklist-uri, colegi de camera, raspunsuri la intrebari stupide —
      totul intr-un singur loc. Facut de studenti, pentru studenti.
    </p>
    <div class="flex gap-3 flex-wrap">
      <a href="/register" class="inline-flex items-center gap-2 bg-bazar-dark text-white px-8 py-3.5 rounded-[14px] font-semibold text-base no-underline hover:-translate-y-0.5 hover:shadow-lg transition-all">
        Incepe acum &rarr;
      </a>
      <a href="/checklist" class="inline-flex items-center gap-2 border-2 border-bazar-gray-300 text-bazar-gray-700 px-6 py-3.5 rounded-[14px] font-medium text-base no-underline hover:border-bazar-dark hover:text-bazar-dark transition-all">
        Vezi cum arata
      </a>
    </div>
  </div>

  <div class="relative order-first md:order-last">
    <span class="absolute -top-2.5 -left-5 text-[32px] -rotate-[15deg] drop-shadow-md z-10">📋</span>
    <span class="absolute top-10 -right-7 text-[32px] rotate-[10deg] drop-shadow-md z-10">🏠</span>
    <span class="absolute bottom-10 -left-7 text-[32px] rotate-[8deg] drop-shadow-md z-10">🤝</span>

    <div class="bg-white rounded-[32px] p-4 shadow-[0_4px_6px_rgba(0,0,0,0.04),0_24px_48px_rgba(0,0,0,0.08)] max-w-[340px] md:max-w-[340px] max-w-[280px] mx-auto rotate-2 md:rotate-2 border-[3px] border-bazar-gray-100">
      <div class="bg-bazar-offwhite rounded-[20px] p-5 min-h-[460px]">
        <div class="font-heading font-bold text-xl mb-1">Checklist-ul tau</div>
        <div class="text-[13px] text-bazar-gray-500 mb-4">3 din 10 completate</div>
        <div class="bg-bazar-gray-100 rounded-full h-2 mb-5 overflow-hidden">
          <div class="bg-bazar-green h-full w-[30%] rounded-full"></div>
        </div>

        <!-- Checklist items -->
        <div class="flex items-start gap-2.5 p-3 bg-white rounded-bazar-sm border-[1.5px] border-bazar-gray-100 mb-2">
          <div class="w-5 h-5 rounded-md bg-bazar-green border-2 border-bazar-green text-white text-[11px] flex items-center justify-center shrink-0 mt-0.5">&#10003;</div>
          <div>
            <div class="text-[13px] font-semibold leading-tight">Confirma locul la facultate</div>
            <span class="inline-block text-[10px] font-semibold px-2 py-0.5 rounded-full mt-1 bg-blue-100 text-blue-800">Acte</span>
          </div>
        </div>
        <div class="flex items-start gap-2.5 p-3 bg-white rounded-bazar-sm border-[1.5px] border-bazar-gray-100 mb-2">
          <div class="w-5 h-5 rounded-md bg-bazar-green border-2 border-bazar-green text-white text-[11px] flex items-center justify-center shrink-0 mt-0.5">&#10003;</div>
          <div>
            <div class="text-[13px] font-semibold leading-tight">Depune cererea pentru camin</div>
            <span class="inline-block text-[10px] font-semibold px-2 py-0.5 rounded-full mt-1 bg-green-100 text-green-800">Cazare</span>
          </div>
        </div>
        <div class="flex items-start gap-2.5 p-3 bg-white rounded-bazar-sm border-[1.5px] border-bazar-gray-100 mb-2">
          <div class="w-5 h-5 rounded-md bg-bazar-green border-2 border-bazar-green text-white text-[11px] flex items-center justify-center shrink-0 mt-0.5">&#10003;</div>
          <div>
            <div class="text-[13px] font-semibold leading-tight">Plateste taxa de inmatriculare</div>
            <span class="inline-block text-[10px] font-semibold px-2 py-0.5 rounded-full mt-1 bg-blue-100 text-blue-800">Acte</span>
          </div>
        </div>
        <div class="flex items-start gap-2.5 p-3 bg-white rounded-bazar-sm border-[1.5px] border-bazar-gray-100 mb-2">
          <div class="w-5 h-5 rounded-md border-2 border-bazar-gray-300 shrink-0 mt-0.5"></div>
          <div>
            <div class="text-[13px] font-semibold leading-tight">Fa-ti adeverinta medicala</div>
            <span class="inline-block text-[10px] font-semibold px-2 py-0.5 rounded-full mt-1 bg-blue-100 text-blue-800">Acte</span>
          </div>
        </div>
        <div class="flex items-start gap-2.5 p-3 bg-white rounded-bazar-sm border-[1.5px] border-bazar-gray-100 mb-2">
          <div class="w-5 h-5 rounded-md border-2 border-bazar-gray-300 shrink-0 mt-0.5"></div>
          <div>
            <div class="text-[13px] font-semibold leading-tight">Exploreaza campusul</div>
            <span class="inline-block text-[10px] font-semibold px-2 py-0.5 rounded-full mt-1 bg-amber-100 text-amber-800">Campus</span>
          </div>
        </div>
      </div>
    </div>
  </div>
</section>

<!-- Features -->
<section id="features" class="max-w-[1200px] mx-auto px-5 md:px-8 mt-10 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
  <div class="bg-white rounded-bazar-lg p-7 border-2 border-bazar-gray-100 hover:border-bazar-yellow hover:-translate-y-1 hover:shadow-lg transition-all">
    <div class="w-12 h-12 rounded-[14px] bg-amber-100 flex items-center justify-center text-2xl mb-4">📋</div>
    <h3 class="font-heading font-bold text-[17px] mb-1.5">Checklist pas cu pas</h3>
    <p class="text-sm text-bazar-gray-500 leading-relaxed">Toate deadline-urile si actele de care ai nevoie, intr-o lista simpla.</p>
  </div>
  <div class="bg-white rounded-bazar-lg p-7 border-2 border-bazar-gray-100 hover:border-bazar-yellow hover:-translate-y-1 hover:shadow-lg transition-all">
    <div class="w-12 h-12 rounded-[14px] bg-violet-100 flex items-center justify-center text-2xl mb-4">🏠</div>
    <h3 class="font-heading font-bold text-[17px] mb-1.5">Gaseste coleg de camera</h3>
    <p class="text-sm text-bazar-gray-500 leading-relaxed">Quiz de compatibilitate + filtre. Afla daca va potriviti.</p>
  </div>
  <div class="bg-white rounded-bazar-lg p-7 border-2 border-bazar-gray-100 hover:border-bazar-yellow hover:-translate-y-1 hover:shadow-lg transition-all">
    <div class="w-12 h-12 rounded-[14px] bg-green-100 flex items-center justify-center text-2xl mb-4">💬</div>
    <h3 class="font-heading font-bold text-[17px] mb-1.5">Chat cu colegii</h3>
    <p class="text-sm text-bazar-gray-500 leading-relaxed">Grupuri pe facultate create automat. Cunoaste-ti colegii.</p>
  </div>
  <div class="bg-white rounded-bazar-lg p-7 border-2 border-bazar-gray-100 hover:border-bazar-yellow hover:-translate-y-1 hover:shadow-lg transition-all">
    <div class="w-12 h-12 rounded-[14px] bg-rose-100 flex items-center justify-center text-2xl mb-4">❓</div>
    <h3 class="font-heading font-bold text-[17px] mb-1.5">Intreaba orice</h3>
    <p class="text-sm text-bazar-gray-500 leading-relaxed">Pune intrebari. Studentii din anii mari raspund.</p>
  </div>
</section>

<!-- Social proof -->
<section class="max-w-[1200px] mx-auto px-5 md:px-8 mt-16 text-center">
  <p class="text-sm text-bazar-gray-500 font-medium mb-5">Disponibil pentru studentii de la:</p>
  <div class="flex justify-center gap-4 md:gap-8 flex-wrap">
    <div class="flex items-center gap-2 px-5 py-3 bg-white rounded-[14px] border-2 border-bazar-gray-100 font-semibold text-[15px] text-bazar-gray-700">
      <span class="w-2.5 h-2.5 rounded-full bg-bazar-green"></span> UPB — Politehnica
    </div>
    <div class="flex items-center gap-2 px-5 py-3 bg-white rounded-[14px] border-2 border-bazar-gray-100 font-semibold text-[15px] text-bazar-gray-700">
      <span class="w-2.5 h-2.5 rounded-full bg-bazar-green"></span> ASE
    </div>
    <div class="flex items-center gap-2 px-5 py-3 bg-white rounded-[14px] border-2 border-bazar-gray-100 font-semibold text-[15px] text-bazar-gray-700">
      <span class="w-2.5 h-2.5 rounded-full bg-bazar-green"></span> Universitatea din Bucuresti
    </div>
  </div>
</section>

<!-- Bottom CTA -->
<section class="relative overflow-hidden bg-bazar-dark rounded-[28px] mx-5 md:mx-8 max-w-[1136px] lg:mx-auto mt-20 mb-16 px-6 md:px-8 py-12 text-center">
  <div class="absolute -top-1/2 -right-[20%] w-[300px] h-[300px] bg-bazar-purple opacity-15 rounded-full blur-[60px]"></div>
  <div class="absolute -bottom-[30%] -left-[10%] w-[250px] h-[250px] bg-bazar-yellow opacity-12 rounded-full blur-[60px]"></div>
  <h2 class="font-heading font-bold text-3xl md:text-4xl text-white mb-3 relative z-10">Gata cu panica de boboc.</h2>
  <p class="text-white/60 text-base mb-7 relative z-10">Fa-ti cont in 30 de secunde. Gratuit.</p>
  <a href="/register" class="relative z-10 inline-flex items-center gap-2 bg-bazar-yellow text-bazar-dark px-8 py-3.5 rounded-[14px] font-bold text-base no-underline hover:-translate-y-0.5 transition-transform">
    Creeaza cont &rarr;
  </a>
</section>

<!-- Footer -->
<footer class="text-center px-5 md:px-8 pb-10 pt-6 text-[13px] text-bazar-gray-500">
  Bazar &copy; 2026 &middot; Facut cu nervi si cafea in Bucuresti
</footer>
